...
### Account creation is on by default + fail-fast gate (2026-07-08)
`auto_create_accounts` **defaults to `true`** (dataclass field and `from_dict` both default on; template ships `true`). Account creation is the only path past the login wall on account-gated platforms (Workday et al.), and it needs an email confirmation code fetched over Gmail IMAP. So `ApplicantProfile.apply_prerequisite_error()` returns a fatal message whenever `auto_create_accounts` is on but `gmail_app_password` doesn't resolve (note it resolves to None unless `auto_fetch_verification_codes` is also true). Every apply entry point calls the gate and exits non-zero before launching a browser: Q1 `cli._abort_if_setup_incomplete` (batch + `--external-url`), Q2 `queue_runner._require_apply_setup` (`process_next_pending` + `process_by_id`). Exempt paths (no gate): `--market-snapshot`, `--setup`, `--sync-profile`, `--deep-apply`, `--list`, dashboard, batch_analysis. The escape hatch in the error message is `auto_create_accounts: false`. This replaced the old silent behavior where a missing gmail password just dropped account-gated jobs as `login_wall`.
