...
| `Setup incomplete: auto_create_accounts is on but no usable Gmail App Password` (apply command exits immediately) | Account creation is on by default and needs a Gmail App Password to receive the confirmation code | Set `application_settings.gmail_app_password` + `auto_fetch_verification_codes: true` (profile `email` must be Gmail), **or** set `auto_create_accounts: false` to skip account-gated platforms. |
| Workday jobs fail as `login_wall` even with account creation on | `gmail_app_password` set but `auto_fetch_verification_codes` is off, so the code fetch never runs | Set `application_settings.auto_fetch_verification_codes: true`. |
